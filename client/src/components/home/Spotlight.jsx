...
import { features as data } from "../../context";
...
const Spotlight = () => {
  useGSAP(() => {
    ...
    features.forEach((feature, index) => {
      const featurePos = featuresStartPosition[index];
      gsap.set(feature, {
        top: `${featurePos.top}%`,
    ...
    ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        if (progress <= 0.3333) {
          const spotlightProgress = progress / 0.3333;
          gsap.set(".spotlight-content", {
            y: `${-100 * spotlightProgress}%`,
          });
        } else {
          gsap.set(".spotlight-content", {
            y: `-100%`,
        ...
        if (progress >= 0 && progress <= 0.5) {
          const featuresProgress = progress / 0.5;
          features.forEach((feature, index) => {
            const original = featuresStartPosition[index];
            const currentTop =
              original.top + (50 - original.top) * featuresProgress;
            const currentLeft =
              original.left + (50 - original.left) * featuresProgress;
            gsap.set(feature, {
              top: `${currentTop}%`,
              left: `${currentLeft}%`,
            });
          ...
          featureBgs.forEach((bg, index) => {
            ...
            const currentWidth =
              featureDim.width +
              (targetWidth - featureDim.width) * featuresProgress;
            const currentHeight =
              featureDim.height +
              (targetHeight - featureDim.height) * featuresProgress;
            const currentBorderRadius = 0.5 + (25 - 0.5) * featuresProgress;
            const currentBorderWidth =
              0.125 + (0.35 - 0.125) * featuresProgress;
          ...
          if (progress >= 0 && progress <= 0.1) {
            const featureOpacityProgress = progress / 0.1;
            gsap.set(".feature-content", {
              opacity: 1 - featureOpacityProgress,
            });
  ...
  return (
    <section className="spotlight text-white">
      <div className="spotlight-content">
        ...
        <h1>Information flows best throught intentional design</h1>
      </div>

      <div className="header">
        <div className="header-content">
          <h1>Find what matters throught intelligent design</h1>
          <p>Discover a system that adapts to the way you thinks.</p>
        </div>
      </div>
