...
const Analytics = () => {
  ...
  const performanceData = modelInfo?.performance
    ? [
        {
          metric: "Accuracy",
          value: Math.round(modelInfo.performance.accuracy * 100),
          benchmark: 95,
        },
        {
          metric: "Precision",
          value: Math.round(modelInfo.performance.precision * 100),
          benchmark: 90,
        },
        {
          metric: "Recall",
          value: Math.round(modelInfo.performance.recall * 100),
          benchmark: 92,
        },
        {
          metric: "F1-Score",
          value: Math.round(modelInfo.performance.f1_score * 100),
          benchmark: 93,
        },
        {
          metric: "ROC-AUC",
          value: Math.round(modelInfo.performance.roc_auc * 100),
          benchmark: 95,
        },
      ]
    : [
      ...
      <header className="sticky top-0 z-50 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-6 py-3">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <nav className="flex items-center gap-8">
            ...
            <Link
              className="text-sm font-medium text-[#0056b2] transition-colors"
              to="/analytics"
            >
              Analytics
            </Link>
        ...
        <header className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm">
              Reviewing performance data for the CKD Risk Prediction Engine.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-[#0056b2]/10 shadow-sm">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className="material-icons text-[#0056b2]/40">
                verified_user
              </span>
            </div>
            <div className="flex items-baseline gap-2">
              <h3 className="text-3xl font-bold text-slate-900 dark:text-slate-100">
                {modelInfo?.performance?.accuracy
                  ? (modelInfo.performance.accuracy * 100).toFixed(1) + "%"
                  : "93.8%"}
              </h3>
            </div>
            <div className="mt-4 w-full bg-slate-100 dark:bg-slate-800 h-1.5 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: `${modelInfo?.performance?.accuracy ? modelInfo.performance.accuracy * 100 : 93.8}%`,
                }}
              ></div>
            </div>
          </div>
          <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-[#0056b2]/10 shadow-sm">
            ...
            <div className="flex items-baseline gap-2">
              <h3 className="text-3xl font-bold text-slate-900 dark:text-slate-100">
                {modelInfo?.performance?.precision_macro
                  ? modelInfo.performance.precision_macro.toFixed(3)
                  : "0.975"}
              </h3>
            ...
            <div className="mt-4 w-full bg-slate-100 dark:bg-slate-800 h-1.5 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: `${modelInfo?.performance?.precision_macro ? modelInfo.performance.precision_macro * 100 : 97.5}%`,
                }}
              ></div>
            </div>
          </div>
          <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-[#0056b2]/10 shadow-sm">
            ...
            <div className="flex items-baseline gap-2">
              <h3 className="text-3xl font-bold text-slate-900 dark:text-slate-100">
                {modelInfo?.performance?.f1_macro
                  ? modelInfo.performance.f1_macro
                  : "4,102"}
              </h3>
            </div>
            <div className="mt-4 w-full bg-slate-100 dark:bg-slate-800 h-1.5 rounded-full overflow-hidden">
              <div
                className="bg-sky-500 h-full transition-all duration-500"
                style={{
                  width: `${modelInfo?.performance?.f1_macro ? modelInfo.performance.f1_macro * 100 : 97.5}%`,
                }}
              ></div>
            </div>
          </div>
          <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-[#0056b2]/10 shadow-sm">
            ...
            <div className="flex items-baseline gap-2">
              <h3 className="text-3xl font-bold text-slate-900 dark:text-slate-100">
                {modelInfo?.features_count || "20538"}
              </h3>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Most influential clinical parameters
              </CardDescription>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
