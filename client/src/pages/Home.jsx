    ...
    <div className="bg-[#f5f7f8] dark:bg-[#0f1923] font-display text-slate-900 dark:text-slate-100 min-h-screen">
      ...
      <header className="sticky top-0 z-50 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-6 py-3">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <nav className="flex items-center gap-8">
            <a
              className="text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-[#0056b2] transition-colors"
              href="#"
            >
              Dashboard
            </a>
            ...
            <div className="flex items-center gap-3">
              {health ? (
                <div
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border ${health.ok ? "bg-[#0056b2]/10 text-[#0056b2] border-[#0056b2]/20" : "bg-red-50 text-red-600 border-red-200"}`}
                >
                  <span className="material-symbols-outlined text-sm">
                    {health.ok ? "cloud_done" : "cloud_off"}
                  </span>
      ...
      <div className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-6 py-2">
        <div className="max-w-7xl mx-auto flex flex-wrap items-center gap-4">
          <div
            className={`flex items-center gap-2 ${health?.ok ? "text-emerald-600 dark:text-emerald-400" : "text-red-500"}`}
          >
            <span className="material-symbols-outlined text-lg">
              {health?.ok ? "check_circle" : "error"}
            </span>
            <span className="text-xs font-semibold uppercase tracking-wider">
              {health?.ok ? "System Ready" : "System Down"}
            </span>
          </div>
          <div className="h-4 w-px bg-slate-300 dark:bg-slate-700"></div>
          <p className="text-xs text-slate-500 dark:text-slate-400 italic">
            Active Engine:{" "}
            {health?.model_name || "Logistic Regression Model (v4.2.1)"} • Last
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700 flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Model validated on 15k+ clinical records
              </p>
            </div>
          </div>
          <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700 flex items-start gap-4">
          ...
          <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700 flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Instant risk probability calculations
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
              <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50">
                ...
                <div className="flex gap-2">
                  <button
                    onClick={fillHealthySample}
                    className="flex items-center gap-2 px-3 py-1.5 text-xs font-semibold bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800 rounded-lg hover:bg-emerald-100 dark:hover:bg-emerald-900/50 transition-all cursor-pointer"
                  >
                    <span className="material-symbols-outlined text-sm">
                      health_and_safety
                    </span>
                  ...
                  <button
                    onClick={fillSampleData1}
                    className="flex items-center gap-2 px-3 py-1.5 text-xs font-semibold bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-600 transition-all cursor-pointer"
                  >
                    <span className="material-symbols-outlined text-sm">
                      content_paste_go
                    </span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1.5 flex flex-col">
                      ...
                      <input
                        ...
                        className="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 p-2 focus:ring-[#0056b2] focus:border-[#0056b2] outline-none"
                        placeholder="e.g. 48"
                        type="number"
                      />
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="space-y-1.5 flex flex-col">
                      ...
                      <select
                        ...
                        value={formData.sg}
                        onChange={handleChange}
                        className="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 p-2 outline-none"
                      >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="space-y-1.5 flex flex-col">
                      ...
                      <input
                        ...
                        value={formData.bgr}
                        onChange={handleChange}
                        className="w-full rounded-lg border border-slate-200 bg-slate-50 dark:bg-slate-900 dark:border-slate-700 p-2 outline-none"
                        type="number"
                      />
                    </div>
                    <div className="space-y-1.5 flex flex-col">
                ...
                <section>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input
                        name="htn"
                        checked={formData.htn === "yes"}
                        onChange={handleChange}
                        type="checkbox"
                        className="rounded text-[#0056b2] focus:ring-[#0056b2] w-5 h-5 cursor-pointer"
                      />
                    ...
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input
                        name="appet"
                        checked={formData.appet === "poor"}
                        onChange={(e) =>
                          setFormData((p) => ({
                            ...p,
                            appet: e.target.checked ? "poor" : "good",
                          }))
                        }
                        type="checkbox"
                        className="rounded text-[#0056b2] focus:ring-[#0056b2] w-5 h-5 cursor-pointer"
              ...
              <div className="p-6 bg-slate-50 dark:bg-slate-800/80 border-t border-slate-100 dark:border-slate-700">
                ...
                {error && (
                  <p className="text-red-500 text-sm mt-3 text-center font-medium bg-red-50 p-2 rounded">
                    {error}
                  </p>
                )}
              </div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden sticky top-24">
              ...
              <div className="p-6 space-y-6">
                {!prediction ? (
                  <div className="text-center py-12 px-4 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl">
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                      Input patient parameters and run the prediction engine to
                      see results here.
                    </p>
                  </div>
                ) : (
                  <>
                    {/* Risk Alert */}
                    <div
                      className={`border p-5 rounded-xl text-center ${
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-100 dark:border-slate-700 text-center">
                        ...
                        <p className="text-2xl font-black text-[#0056b2]">
                      ...
                      <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-100 dark:border-slate-700 text-center">
                        ...
                        <p
                          className={`text-2xl font-black ${prediction.predicted_class === 1 ? "text-red-600" : "text-emerald-600"}`}
                        >
                        ...
                        <div>
                          <div className="flex justify-between text-sm mb-1">
                            ...
                            <span className="font-bold">
                              {(
                                (prediction.probabilities?.ckd || 0) * 100
                              ).toFixed(1)}
                              %
                            </span>
                          </div>
                          <div className="w-full bg-slate-200 dark:bg-slate-700 h-2 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${(prediction.probabilities?.ckd || 0) * 100}%`,
                              }}
                            ></div>
                          </div>
                        </div>
                    ...
                    <div className="pt-6 border-t border-slate-100 dark:border-slate-700 space-y-4">
                      <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                        ...
                        <span>
                          {prediction.model || "Machine Learning API"}
                        </span>
                      </div>
                      <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                ...
                <div className="bg-[#0056b2]/5 rounded-lg p-4 flex gap-3 items-start border border-[#0056b2]/10 mt-4">
                  <span className="material-symbols-outlined text-[#0056b2] text-lg shrink-0">
                    info
                  </span>
                  <p className="text-[11px] leading-relaxed text-slate-600 dark:text-slate-400">
                    <span className="font-bold text-[#0056b2]">
                      Disclaimer:
                    </span>{" "}
                    This AI tool is for preliminary screening and educational
                    purposes only. Results must be verified by a certified
                    medical professional and are not a final diagnosis.
                  </p>
                </div>
              </div>
            </div>
      ...
      <footer className="max-w-7xl mx-auto px-6 py-10 mt-8 border-t border-slate-200 dark:border-slate-800 text-center">
        <p className="text-xs text-slate-500 dark:text-slate-400">
          © {new Date().getFullYear()} CKD Prediction. Powered by Advanced
          Clinical Analytics.
        </p>
      </footer>
    </div>
